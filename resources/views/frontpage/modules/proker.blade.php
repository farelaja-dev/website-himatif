...
@section('content')
    ...
    <main>
            ...
            <div class="container relative z-10 mx-auto flex justify-center">
                <div class="flex flex-col items-center">
                    <div class="inline-block px-8 py-4 mb-6 text-white rounded-full bg-[#910E19] opacity-0 translate-y-8 transition-all duration-1000 ease-out" data-animate>
                        <span class="text-xl md:text-6xl font-semibold">Program Kerja</span>
                    </div>
                    ...
                    <p class="max-w-6xl text-lg leading-relaxed text-gray-600 text-center opacity-0 translate-y-8 transition-all duration-1000 ease-out delay-400" data-animate>
       ...
       <!-- Program Kerja Section -->
            ...
            <div class="container mx-auto">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    @foreach ($prokers as $proker)
                        @php
                            ...
                            if (isset($proker->prokerUsers) && $proker->prokerUsers->count() > 0) {
                                ...
                                if ($firstUser && $firstUser->prokerDivision) {
                                    $divisionName = $firstUser->prokerDivision->name ?? 'PSDM';
                                }
                            }
                        ...
                        <a href="{{ route('frontpage.proker.show', $proker->id) }}" class="relative block group opacity-0 translate-y-8 transition-all duration-1000 ease-out" data-animate>
                            
                            <!-- Card -->
                            <div class="overflow-hidden shadow-lg proker-card 
                                        rounded-br-[8rem] rounded-t-3xl rounded-bl-3xl 
                                        hover:shadow-2xl transition-all duration-300 
                                        h-[420px] flex flex-col bg-[#910E19]">
                                
                                <!-- Logo Icon -->
                                <div class="absolute top-6 left-1/2 transform -translate-x-1/2 w-38 h-38 flex items-center justify-center">
                                    @if ($proker->logo)
                                        <!-- <img src="{{ asset('storage/' . $proker->logo) }}" alt="{{ $proker->name }}" class="w-full h-full object-cover"> -->
                                        <img src="{{ asset('img/bagian/h-placeholder.png') }}" alt="placeholder" class="w-32 h-32 object-contain">
                                    @else
                                        <img src="{{ asset('img/bagian/h-placeholder.png') }}" alt="placeholder" class="w-32 h-32 object-contain">
                                    @endif
                                </div>
                                ...
                                <div class="relative z-10 flex flex-col flex-1 p-6">
                                    <!-- Title -->
                                    <h3 class="mt-32 mb-4 text-3xl font-bold text-white">{{ $proker->name }}</h3>
                                    ...
                                    <div class="flex items-end justify-between mt-auto">
                                        <span class="px-4 py-2 text-sm font-bold tracking-wide text-gray-900 bg-white rounded-full">
                                            {{ $divisionName }}
                                        </span>
                                    </div>
                                    ...
                                    @if ($proker->is_registration_open === '1')
                                        <div class="absolute top-6 right-6">
                                            <span class="inline-block px-3 py-1 text-xs font-semibold text-green-900 bg-green-200 rounded-full">
                                                Buka
                                            </span>
                                        </div>
                                    @endif
                            ...
                            <div class="absolute bottom-10 right-5 z-20 
                                ...
                                <svg class="w-8 h-8 text-gray-900 transition-transform duration-300 
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17 17L7 7M7 7H17M7 7V17"></path>
                                </svg>
                            </div>
        ...
        <div class="w-full h-full bg-[#FEF9F1] opacity-0 translate-y-8 transition-all duration-1000 ease-out" data-animate>
